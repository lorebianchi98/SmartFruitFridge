...
#define SENSOR_TYPE "ripening_notifier"
...
PROCESS(ripening_notifier_server, "Server for ripening notifier");
AUTOSTART_PROCESSES(&ripening_notifier_server);
...
extern coap_resource_t ripening_notifier;
...
PROCESS_THREAD(ripening_notifier_server, ev, data)
{
  ...
  etimer_set(&wait_connectivity, CLOCK_SECOND* CONNECTION_TRY_INTERVAL);
	...
	while(!registered){
		...
		LOG_INFO("[RIPENING NOTIFIER] sending registration message\n");	
	...
	LOG_INFO("Starting ripening notifier Server\n");
	
	coap_activate_resource(&ripening_notifier, "ripening_notifier");  
